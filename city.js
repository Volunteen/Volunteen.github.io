...
function setupMap(placeList){
	var markers = new L.MarkerClusterGroup({animateAddingMarkers: true, spiderfyOnMaxZoon: true, showCoverageOnHover:true, zoomToBoundsOnClick: true});
	myLayer.setGeoJSON(placeList);
	map.fitBounds(myLayer.getBounds());
	markers.addLayer(myLayer);
	map.addLayer(markers);
	
	markers.on('clusterclick', function (a) {a.layer.zoomToBounds(); setUpMap(placeList) });
